 ...
 */
template<>
InputParameters validParams<CureFormulaS3>()
{
...
CureFormulaS3::CureFormulaS3(const InputParameters & parameters) :
...
Real CureFormulaS3::computeQpResidual()
{
  ...
  */
  Real A=35;
  Real B=1;
  return  -_test[_i][_qp]*B*exp(-A/_v_var[_qp])*_u[_qp]*pow((1-_u[_qp]),2); //i changed this
    //note that _v_var[_qp] that represents arrhenius temperature dependence has been added
  //e^(-A/t)a(1-a)^2
}

Real CureFormulaS3::computeQpJacobian()
{
  ...
  */
  ...
  Real A=35;
  Real B=1;
  return -_test[_i][_qp]*B*exp(-A/_v_var[_qp])*(pow((1-_u[_qp]),2) -2*_u[_qp]*pow((1-_u[_qp]),1))*_phi[_j][_qp];
}

//off diagonal Jacobian needs to be added if two way coupling is there !!!!!!!!!!

Real CureFormulaS3::computeQpOffDiagJacobian(unsigned int jvar)
{
  Real A=35;
  Real B=1;
  cout<<"offDiagonalJacobian executed"<<endl;
    ...
    if (jvar==_T_id)
    {
      return  -_test[_i][_qp]*B*(exp(-A/_v_var[_qp])*(A/pow(_v_var[_qp],2))*_phi[_j][_qp])*_u[_qp]*pow((1-_u[_qp]),2);
     //_v_var[_qp] from residual becomes _phi[_j][_qp] in the off diagonal jacobian
      //Chain rule of differentiation used here
    }
